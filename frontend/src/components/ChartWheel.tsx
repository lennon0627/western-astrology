...
import { ASPECT_INTERP } from '@/lib/interp'
import type { ChartResponse, Aspect } from '@/lib/types'
...
export default function ChartWheel({ chart, size = 500 }: Props) {
  ...
  return (
    <div className="flex flex-col items-center gap-3">
          ...
          return (
            <g key={i}>
              ...
              <line
                ...
                stroke={color}
                strokeWidth={isActive ? 2.5 : (isHard ? 1.2 : 0.8)}
              ...
              <line
                ...
                onMouseEnter={() => setActiveAspect(a)}
                onClick={() => setActiveAspect(prev => prev === a ? null : a)}
              />
            </g>
          )
        })}
      ...
      {/* ─── アスペクト詳細パネル ─────────────────────────────────────────────── */}
      <div className={`w-full max-w-sm transition-all duration-200 ${activeAspect ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        {activeAspect && (() => {
          const info = ASPECT_INTERP[activeAspect.aspect]
          const color = ASPECT_COLORS[activeAspect.aspect] ?? '#64748b'
          const p1Color = PLANET_COLORS[activeAspect.planet1] ?? '#ffffff'
          const p2Color = PLANET_COLORS[activeAspect.planet2] ?? '#ffffff'
          return (
            <div className="rounded-lg border border-border bg-card px-4 py-3 text-sm space-y-2">
              {/* 天体 × アスペクト × 天体 */}
              <div className="flex items-center justify-center gap-2 font-semibold">
                <span style={{ color: p1Color }}>
                  {PLANET_SYMBOLS[activeAspect.planet1] ?? activeAspect.planet1}
                  <span className="ml-1 text-xs text-muted-foreground">{activeAspect.planet1_jp}</span>
                </span>
                <span className="text-base" style={{ color }}>{info?.symbol ?? '—'}</span>
                <span style={{ color: p2Color }}>
                  {PLANET_SYMBOLS[activeAspect.planet2] ?? activeAspect.planet2}
                  <span className="ml-1 text-xs text-muted-foreground">{activeAspect.planet2_jp}</span>
                </span>
              </div>
              {/* アスペクト名・orb */}
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                <span style={{ color }}>{info?.jp ?? activeAspect.aspect}</span>
                <span>orb {activeAspect.orb.toFixed(1)}°</span>
                <span className={activeAspect.type === 'ハード' ? 'text-destructive' : 'text-blue-400'}>
                  {activeAspect.type}
                </span>
              </div>
              {/* 解説 */}
              {info && <p className="text-xs text-muted-foreground leading-relaxed">{info.desc}</p>}
            </div>
          )
        })()}
      </div>
